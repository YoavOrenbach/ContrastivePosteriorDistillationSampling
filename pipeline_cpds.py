...
class CPDSPipeline(StableDiffusionPipeline):
    ...
    def __call__(
        ...
        # Define loss class
        if config == "cds" or config == "dds":
            framework_loss = DDSLoss(t_min=self.min_step, t_max=self.max_step, unet=self.unet, scheduler=self.scheduler,device=device)
            optimizer = optim.SGD([z_trg], lr=0.1)
        else:
            framework_loss = PDSLoss(t_min=self.min_step, t_max=self.max_step, unet=self.unet, scheduler=self.scheduler,device=device)
            optimizer = optim.SGD([z_trg], lr=0.8)

        cut_loss = EnhancedCutLoss(device, n_patches=n_patches, patch_size=patch_size, selector_type=patch_selector)
        ...
        cut_loss.plot_losses_accuracies()
    ...
    def _contrasive_loss_step(self, framework_loss, cut_loss, z_src, z_trg, prompt_embeds, trg_prompt_embeds, sa_attn, w_dds, w_cut, device):
        z_t_src, eps, timestep, timestep_prev = framework_loss.noise_input(z_src, eps=None, timestep=None)
        z_t_trg, _, _, _ = framework_loss.noise_input(z_trg, eps, timestep)
        # get score for dds & reference attention maps
        eps_pred = framework_loss.get_epsilon_prediction(
            torch.cat((z_t_src, z_t_trg)),
            torch.cat((timestep, timestep)),
            torch.cat((prompt_embeds, trg_prompt_embeds))
        )
        eps_pred = framework_loss.process_epsilon_prediction(eps_pred, z_trg, z_t_trg, timestep, timestep_prev)
        ...
        with torch.enable_grad():
            z_t_trg, _, _, _ = framework_loss.noise_input(z_trg, eps, timestep)
            eps_pred_trg = framework_loss.get_epsilon_prediction(
               z_t_trg,
               timestep,
               trg_prompt_embeds)
            eps_pred_trg = framework_loss.process_epsilon_prediction(eps_pred_trg, z_trg, z_t_trg, timestep, timestep_prev)
    ...
    def _loss_step(self, framework_loss, z_src, z_trg, prompt_embeds, trg_prompt_embeds, w_dds):
        z_t_src, eps, timestep, timestep_prev = framework_loss.noise_input(z_src, eps=None, timestep=None)
        z_t_trg, _, _, _ = framework_loss.noise_input(z_trg, eps, timestep)
        # get score for dds & reference attention maps
        eps_pred = framework_loss.get_epsilon_prediction(
